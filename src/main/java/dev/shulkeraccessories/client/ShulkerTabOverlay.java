...
public class ShulkerTabOverlay {
    ...
    public ShulkerTabOverlay(int activeSlot) { this.activeSlot = activeSlot; }
    ...
    public void buildTabs() {
        ...
        for (int i = 0; i < shulkerContainer.getSize(); i++) {
            ...
            if (ShulkerAccessoriesMod.isShulkerBox(stack)) {
                ...
                float[] tint = (color == null && SSCompat.isSSShulkerBox(stack))
                        ? null
                        : computeTint(color, 0.5f);
    ...
    public static float[] computeTint(@Nullable DyeColor color, float colorWeight) { return computeTint(color != null ? color.getId() : -1, colorWeight); }
